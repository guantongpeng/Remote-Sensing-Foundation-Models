...
![image-20260414160716500](./assets/image-20260414160716500.png)**OlmoEarth**是由**艾伦人工智能研究所**推出的**时空多模态地球观测基础模型**，基于**ViT架构**设计**Latent MIM Lite**稳定训练机制，搭配**模态感知掩码**与**模态感知损失**，在**24项嵌入任务**中拿下**15项最优**、**29项全微调任务**拿下**19项最优**，并依托**OlmoEarth平台**为公益与环保组织提供端到端工具，开源代码、数据与权重，兼顾**性能-效率最优**与**低环境影响**。

#### 1. 模型背景与核心目标
地球观测数据具备**空间性、时序性、多模态**特征，现有基础模型存在**训练不稳定、部署成本高、公益领域落地难**问题，OlmoEarth旨在打造**稳定高效、开源易用**的地球观测基础模型，赋能非营利组织解决环保、气候、粮食安全等问题。

#### 2. 核心技术创新
##### （1）稳定训练：Latent MIM Lite
- 简化传统Latent MIM，用**随机初始化且冻结**的线性投影层替代目标编码器
- 避免**表征崩溃**，统一自监督与监督学习流程
##### （2）模态感知掩码
- 结合随机令牌掩码与全模态重建，无需超高掩码比例即可提升任务难度
- 地图数据仅用于解码，观测数据可编码/解码/两者兼具
##### （3）专属损失函数
...
- **作用**：作为监督学习的目标，让模型学习有明确语义的地理特征
- **对应你之前的问题**：**这一整块就是监督学习的数据来源**，后续会通过「Random Project」生成固定的监督目标
...
翻译：
**随机投影在理论和实践上都能提取有效特征；在自然图像可能太简单，但在地球观测数据上效果显著、非常稳。**
...
3. **模态感知设计**：不同模态（卫星/地图）用不同颜色区分，掩码策略、损失计算都做了模态适配
4. **时空建模**：Patchify 是三维时空分块，Encoder/Decoder 天然支持时序特征学习，适配地球观测的时间序列特性

---

##### 四、再明确「监督学习」的完整链路（对应图中位置）
在这张图里，监督学习的完整链路是：
...
两者在 `Patch Contrastive Loss` 处汇合，用同一个损失完成两种学习，这就是 OlmoEarth 最核心的设计。
